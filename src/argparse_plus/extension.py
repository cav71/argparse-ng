...
class ExtensionBase:
    NAME = None # an extension can define a unique name or have it defined by the __name__
    ...
    def get(cls, name):
        ...
        def walks(node):
            for subclass in node.__subclasses__():
                ...
                if sname.lower() in classes:
                    raise DuplicateExtensionNameError(f"duplicate subclass with name='{sname}' found",
                                                      sname, subclass, classes[sname.lower()])
        ...
        if key not in classes:
            raise MissingExtensionNameError(f"cannot find extension {name}", name, key, list(classes))
